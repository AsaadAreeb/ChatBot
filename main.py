...
from evaluate import runTest_BeamSearch, runTest_GreedySearch
...
def parse():
    ...
    parser.add_argument('-g', '--greedy', type=bool, default=True, help='During Testing use greedy search')
...
def run(args):
    ...
    if args.train and not args.load:
        ...
        if args.greedy:
            n_layers, hidden_size, _ = parseFilename(args.test, True)
            runTest_GreedySearch(n_layers, hidden_size, args.test, inp, args.corpus)
        else:
            n_layers, hidden_size, reverse = parseFilename(args.test, True)
            runTest_BeamSearch(n_layers, hidden_size, reverse, args.test, beam_size, inp, args.corpus)
